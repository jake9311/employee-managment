...
exports.createGuard = async (req, res) => {
  try {
    if (!req.user) return res.status(403).json({ error: 'Unauthorized- no user info' });
        ...
        for (const u of userInOrg) {
          if (u.email) {
            try {
              await sendEmail(
                u.email,
                `תזכורת להבאת אישור מחלה התראה:`,
                `  ${guard.name} סיים מחלה לפני ${differentDays} ימים ולא הביא אישור מחלה`
              );
              sentAtLeastOne = true;
              console.log(
                `התראה: מאבטח ${guard.name} סיים מחלה לפני ${differentDays} ימים ולא הביא אישור מחלה`
              );
            } catch (err) {
...
exports.getLastReports = async (req, res) => {
  try {
    ...
    const reports = guards.map((guard) => {
      ...
      const sick = guard.sickDays.map((sickDay) => ({
        type: 'מחלה',
        sickDayId: sickDay._id,
        startDate: sickDay.startDate,
        endDate: sickDay.endDate,
        reason: sickDay.reason,
        guardName: guard.name,
        guardId: guard._id,
        hasApproval: sickDay.hasApproval,
        notified: sickDay.notified,
